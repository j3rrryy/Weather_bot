...
async def week_forecast_days(callback: CallbackQuery):
    ...
    if lang == 'RU':
        await callback.message.edit_text(LEXICON_RU["weather_week"], reply_markup=days_kb(lang))
    else:
        await callback.message.edit_text(LEXICON_EN["weather_week"], reply_markup=days_kb(lang))
